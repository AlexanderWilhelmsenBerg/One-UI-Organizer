...
These rules apply to **all implementation, refactoring, testing, build, CI, and dependency work** in this repository.
...
- Do not create generic god abstractions such as `AndroidManager`.
...
Use stable AndroidX Macrobenchmark for real user-flow performance once the flow exists. Use Microbenchmark only for isolated hot code. Do not introduce pre-release benchmark/profile tooling to bypass the stable-only rule.
