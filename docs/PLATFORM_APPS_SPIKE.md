...
`AndroidAppLauncher` validates the frozen target through a small pure `LaunchComponentSpec` seam, then builds a main-launcher intent for that exact `ComponentName`. It returns `false` for blank target identity, missing activities, or security rejection.
...
3. Compare the diagnostic launcher-target count/list against ordinary user-launchable apps visible in One UI. Record any material omissions and whether they are explained by Android package visibility/profile behavior.
